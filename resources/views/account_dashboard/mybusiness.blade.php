...
@include('account_dashboard.dashboardSidebar')
    <main class="col-lg-9">
      <div class="main-content">
        <h5 class="page-title">BUSINESS PROFILE</h5>
        ...
        <div class="investor-profile-section">
          <div class="investor-images">
            @if(!empty($user->profile_pic) && file_exists(public_path($user->profile_pic)))
              <img src="{{ asset($user->profile_pic) }}" alt="Business" class="investor-image-large">
            @else
              <div class="investor-image-fallback">{{ strtoupper(substr($user->name ?? 'B', 0, 1)) }}</div>
            @endif
          </div>

          <div class="investor-details">
            <h4>{{ $profile->advmt_headline ?? 'Not Set' }}</h4>
            <p class="text-muted">{{ $profile->seller_intro ?? 'Not Set' }}</p>
            ...
            <div class="detail-row"><span class="detail-label">Company Name:</span><span class="detail-value">{{ $profile->seller_company ?? 'Not Set' }}</span></div>
